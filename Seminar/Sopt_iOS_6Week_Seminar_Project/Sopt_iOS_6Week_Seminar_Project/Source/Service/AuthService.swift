...
struct AuthService {
    ...
    func signUp(email: String, password: String, username: String, completion: @escaping (NetworkResult<Any>) -> ()) {
        ...
        let body: Parameters = ["email": email, "password":password, "userName":username]
        ...
        dataRequest.responseData { (response) in
            ...
            case .success :
                ...
                completion(judgeSingUpData(status: statusCode, data: data))
    ...
    private func judgeSingUpData(status: Int, data: Data) -> NetworkResult<Any> {
        let decoder = JSONDecoder()
